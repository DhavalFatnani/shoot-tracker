...
export function MultiTaskCsvDownload({ taskIds }: { taskIds: string[] }) {
  const [pending, setPending] = useState(false);
  async function handleDownload() {
    if (taskIds.length === 0) return;
    setPending(true);
    try {
      const result = await exportMultiTaskData(taskIds);
  ...
  return (
    <button
      ...
      disabled={taskIds.length === 0 || pending}
    ...
    >
      ...
      {pending ? "Preparing…" : "Multi-task data (CSV)"}
    </button>
  );
}
