              ...
              {events.length > 0 && (
                <button
                  ...
                  onClick={() => {
                    ...
                    const rows = events.map((ev) => [ev.eventType, ev.fromLocation, ev.toLocation, formatDateTimeIST(ev.createdAt)]);
                    const csv = buildCsv(headers, rows);
                    downloadCsv(csv, `serial-timeline-${displaySerialId}-${new Date().toISOString().slice(0, 10)}.csv`);
                  }}
                  className="inline-flex items-center gap-2 rounded-lg border border-zinc-300 bg-white px-3 py-2 text-sm font-medium text-zinc-700 shadow-sm transition hover:bg-zinc-50 dark:border-zinc-600 dark:bg-zinc-800 dark:text-zinc-300 dark:hover:bg-zinc-700"
                >
