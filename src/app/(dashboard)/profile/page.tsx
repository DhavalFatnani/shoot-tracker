    ...
    <div className="space-y-8">
      ...
      <div className="page-header flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="page-subtitle mt-1">Your account, name, change password, and role details.</p>
        </div>
        <Link href="/profile/change-password" className="btn btn-secondary">
          Change password
        </Link>
      </div>
      ...
      {teamList.length > 0 && (
        <div className="section-card">
          <div className="border-b border-slate-200 px-5 py-4 dark:border-slate-700">
            <h2 className="font-display text-sm font-semibold text-slate-900 dark:text-slate-100">Team membership</h2>
            <p className="mt-0.5 text-xs text-slate-500 dark:text-slate-400">Teams you belong to</p>
          </div>
          <ul className="divide-y divide-slate-100 p-5 dark:divide-slate-600">
            {teamList.map((t) => (
      ...
      {warehouseList.length > 0 && (
        <div className="section-card">
          <div className="border-b border-slate-200 px-5 py-4 dark:border-slate-700">
            <h2 className="font-display text-sm font-semibold text-slate-900 dark:text-slate-100">Warehouse access</h2>
            <p className="mt-0.5 text-xs text-slate-500 dark:text-slate-400">Ops warehouses linked to your teams</p>
          </div>
          <ul className="divide-y divide-slate-100 p-5 dark:divide-slate-600">
            {warehouseList.map((w) => (
              <li key={w.id} className="flex items-center justify-between py-2 first:pt-0 last:pb-0">
                ...
                <span className="font-mono text-xs text-slate-500 dark:text-slate-400">{w.code}</span>
              </li>
            ))}
          </ul>
        </div>
      ...
      {teamList.length === 0 && session.role !== "ADMIN" && (
        <div className="section-card border-amber-200 bg-amber-50 p-5 dark:border-amber-800 dark:bg-amber-900/20">
