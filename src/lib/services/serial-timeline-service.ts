...
export async function getSerialTimeline(serialId: string, limit: number = 100) {
  ...
  const db = getDb();
  const [events, registry, locationRow] = await Promise.all([
    eventRepo.getEventsBySerialId(db, parsed.data, limit),
    serialRegistryRepo.serialRegistryById(db, parsed.data),
    serialStateRepo.getLocation(db, parsed.data),
  ]);
  const currentLocation = locationRow?.currentLocation ?? null;
  return { events, serialId: parsed.data, sku: registry?.sku ?? "", currentLocation };
}
